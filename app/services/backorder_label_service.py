...
def make_backorder_labels(
    ...
    for r in rows:
        ...
        if not hdr:
            log.error("Header bulunamadı: %s", ord_no); continue
        ...
        except Exception as exc:                 # DB hatası
            log.error("Header güncelle hatası %s: %s", ord_no, exc)
            continue
        ...
        try:
            ...
            create_labels(
                ord_no,
                force=force,
                footer="EKSİK GÖNDERİLEN SEVKİYAT"      #  ← EKLENDİ
            )
