...
LABEL_DIR = BASE_DIR / "labels"  # label_service varsayılanı
...
class ShipmentPage(QWidget):
    ...
    def _add_row(self, rec: Dict):
        ...
        for col,(key,_) in enumerate(COLS):
            itm = QTableWidgetItem(str(rec.get(key,""))); itm.setTextAlignment(Qt.AlignCenter)
  ...
  # ------------------------------------------------------------------
    ...
    def _open_or_create_label(self, order_no: str):
        ...
        try:
            upsert_header(order_no      = order_no,
        ...
        if pdf_path and pdf_path.exists():
            prn_name = st.get("print.label_printer", "")   # SettingsPage > Yazdırma sekmesi
            try:
                if prn_name:                               # doğrudan yazıcıya gönder
                    # Windows’ta “print” verb ile gönder; prn_name sistemde varsayılan değilse
                    #   Raw baskı için ek araç gerekebilir, basit çözüm:
                    os.startfile(pdf_path, "print")        # seçilen yazıcı sürücüde default olmalı
                else:
                    os.startfile(pdf_path)                 # ekranda aç
            except Exception as exc:
                QMessageBox.information(self, "PDF",
                                        f"Etiket PDF hazırlandı: {pdf_path}\n{exc}")
        else:
    ...
    def export_pdf(self):
        ...
        # ------ Font ---------------------------------------------------
        try:
            fpath = BASE_DIR / "fonts" / "DejaVuSans.ttf"
            pdfmetrics.registerFont(TTFont("DejaVuSans", str(fpath)))
            FONT = "DejaVuSans"
        except Exception:                       # dosya yok → Helvetica
            FONT = "Helvetica"
