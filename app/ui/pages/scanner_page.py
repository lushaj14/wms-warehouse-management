...
from app.dao.logo import (
    ...
    log_activity,
    queue_inc,
    lookup_barcode,          # ← DAO’daki fonksiyon
)
...
class ScannerPage(QWidget):
    ...
    def _build_ui(self):
        ...
        lay.addWidget(self.btn_done, alignment=Qt.AlignRight)
    ...
    def _populate_table(self):
        ...
        """
        from PyQt5.QtGui import QColor  # gerekliyse üst import bloğuna da ekleyin
    ...
    def on_scan(self) -> None:
        ...
        self.entry.clear()
        if not raw or not self.current_order:
            return

        # 0) Depo kimliği ------------------------------------------------
        wh = self.current_order.get("warehouse_id") or self._infer_wh_from_prefix(raw)
       ...
       # 3) barcode_xref tablosu ---------------------------------------
        if not matched_line:
            # Sipariş satırlarında geçen tüm depoları dene
            for wh_try in {ln["warehouse_id"] for ln in self.lines}:
                ...
                if itm_code:
                    ...
                    if matched_line:            # bulundu ➜ wh, qty_inc ayarla ve çık
                        wh = wh_try
                        qty_inc = float(mult or 1)
                        break


        # 4) Barkod hâlâ bulunamadı -------------------------------------
        if not matched_line:
            snd_err.play()
